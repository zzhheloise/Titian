...
args_15 = get_args('config/train_future/t5_lora15.json')
args_16 = get_args('config/train_future/t5_lora16.json')
args_17 = get_args('config/train_future/t5_lora17.json')
args_18 = get_args('config/train_future/t5_lora18.json')
args_19 = get_args('config/train_future/t5_lora19.json')
args_20 = get_args('config/train_future/t5_lora20.json')
...
dataset_16 = Pretrain(tokenizer=tokenizer, type_path="train", num_samples=None,  input_length=args_15.max_input_length, 
                output_length=args_15.max_output_length, args=args_15, length=None)
dataset_17 = Pretrain(tokenizer=tokenizer, type_path="train", num_samples=None,  input_length=args_16.max_input_length, 
                output_length=args_16.max_output_length, args=args_16, length=None)
dataset_18 = Pretrain(tokenizer=tokenizer, type_path="train", num_samples=None,  input_length=args_17.max_input_length, 
                output_length=args_17.max_output_length, args=args_17, length=None)
dataset_19 = Pretrain(tokenizer=tokenizer, type_path="train", num_samples=None,  input_length=args_18.max_input_length, 
...
loader_16 = DataLoader(dataset_16, batch_size=args_15.train_batch_size, shuffle=False)
loader_17 = DataLoader(dataset_17, batch_size=args_16.train_batch_size, shuffle=False)
loader_18 = DataLoader(dataset_18, batch_size=args_17.train_batch_size, shuffle=False)
loader_19 = DataLoader(dataset_19, batch_size=args_18.train_batch_size, shuffle=False)
...
for epoch in range(args_15.num_train_epochs):
    ...
    for idx, (batch_16, batch_17, batch_18, batch_19) in enumerate(tqdm(tzip(loader_16, loader_17, loader_18, loader_19))):

        # batch_16
        lm_labels_16 = batch_16["target_ids"]
        lm_labels_16[lm_labels_16[:, :] == tokenizer.pad_token_id] = -100
        outputs_16 = model_15(
            input_ids=batch_16["source_ids"].to(device),
            attention_mask=batch_16["source_mask"].to(device),
            lm_labels=lm_labels_16.to(device),
            decoder_attention_mask=batch_16['target_mask'].to(device)
        )
        # normalize loss to account for batch accumulation
        loss = outputs_16[0] / accum_iter
        # backward pass
        loss.backward()
